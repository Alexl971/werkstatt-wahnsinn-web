...
import { fetchTopByGame } from "../lib/supabase";
...
export default function Highscores({ onBack }) {
  ...
  const load = async (game) => {
    setState("loading");
    const { data, error } = await fetchTopByGame(game, 50, 7);
    ...
    <div style={styles.wrap}>
      <div style={styles.header}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button className="btn" style={styles.btn} onClick={() => load(active)}>Aktualisieren</button>
      ...
      <div style={styles.tabs}>
        {GAMES.map((g) => (
          <button
            ...
            style={{
              ...styles.tab,
              background: active === g ? "#2563eb" : "#0b1220",
              borderColor: active === g ? "#2563eb" : "#1f2937",
              color: active === g ? "#fff" : "#e5e7eb",
            }}
          >
            {GAME_LABELS[g]}
          </button>
      ...
      <div style={styles.card}>
        <div style={styles.titleRow}>
          ...
          <div style={{ opacity: 0.8, fontSize: 13 }}>beste pro Spieler • letzte 7 Tage</div>
        </div>
...
const styles = {
  ...
  btnGhost: { background: "#334155", borderRadius: 12, border: "none", color: "#e5e7eb", padding: "8px 12px", cursor: "pointer", fontWeight: 700 },
};